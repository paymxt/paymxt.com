...
const fetchCapabilities = async () => {
  ...
  capabilities.value = data.stories.filter((story) => story.is_startpage !== true)
  loading.value = false
}
    ...
    <section class="my-16 flex text-dark">
      ...
      >
        <div>
          ...
          <input
            ...
            @keypress.enter="fetchCapabilities()"
          />
        </div>
        <!-- <fieldset>
      ...
      >
        <CapabilityCard
          ...
          :slug="capability.full_slug"
          class="bg-light"
        />
      </section>

      <section v-else-if="!loading && !capabilities.length">
        Unfortunately, no capabilities matched your criteria.
      </section>
    </section>
  </main>
</template>
